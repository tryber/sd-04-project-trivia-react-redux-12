...
class OpçoesRespostas extends Component {
  ...
  handleClick1 = () => this.setState({correct:'Correct', wrong:'Wrong', random:'true'});
        ...
        {arrayResposts.map((element, index) => (
            element === objQuestions.correct_answer ? (
              <button
                ...
                className={correct} onClick={() => this.handleClick1()}
              >
                {element}
              </button>
            ) : (
              <button
                ...
                className={wrong} onClick={() => this.handleClick1()}
              >
                {element}
              </button>
            )))}
